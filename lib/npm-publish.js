...
const writeChangelog = require('./shared/write-changelog')
...
const silentExec = cmd => shelljs.exec(cmd, {silent: true})
const fatalExec = cmd => {
  const ret = shelljs.exec(cmd)
  if (ret.code !== 0) bail(cmd)
  return ret
}
...
const GIT_LOG_JSON_FORMAT = `{"hash": "%H", "date": "%aI", "subject": "%s", "body": ${GIT_BODY_DELIMITER}"%B"${GIT_BODY_DELIMITER}}`
...
function getLastVersion(options) {
  ...
  const {stdout, code} = silentExec(`git describe --tags --abbrev=0 ${prereleaseFilter}`)
  if (code === 0) {
    const tag = stdout.trim()
    const hash = silentExec(`git rev-parse --short ${tag}`).stdout.trim()
    return {hash, tag, parsed: semver.parse(tag)}
  }

  const hash = silentExec('git rev-list --max-parents=0 HEAD').stdout.trim()
  return {hash, tag: hash, parsed: semver.parse('0.0.1')}
}

function getNextVersion(options, {lastVersion, releaseType}) {
  const hash = silentExec('git rev-parse --short HEAD').stdout.trim()
  ...
  // treat 0.x bumps as major version bumps
  if (lastVersion.parsed.major === 0 && releaseIncrement === 'major') releaseIncrement = 'minor'
  // force a major increment is told to
  if (process.env.FORCE_MAJOR) releaseIncrement = 'major'

  if (options.prerelease) {
    if (!lastVersion.parsed.prerelease) {
      // this is the first prerelease, do premajor if we have breaking changes, preminor everything else
      ...
      // last release was a prelease for the same major, but now we have breaking changes, force a premajor
...
function getCommitsAndReleaseType(lastVersion) {
  ...
  let logs = silentExec(command).stdout
...
function publishToNPM(options, {nextVersion}) {
  let publishCode
  ...
  if (options.lerna) {
    ...
    code = shelljs.exec(`lerna publish ${lernaArgs.join(' ')}`).code
  } else {
    ...
    code = shelljs.exec(`npm publish --tag ${npmTag}`).code
...
function tagAndPushGitRepo(options, {nextVersion, repository}) {
  const versionWithV = `v${nextVersion.parsed.version}`
  fatalExec(`git tag -a ${versionWithV} -m '${versionWithV}'`)
  fatalExec(`git push origin ${versionWithV}`)
}

function addGitChangelogReleaseNotes(options, )

function fillDefaultOptions(options) {
  options.lerna = typeof options.lerna === 'boolean' ? options.lerna : fs.existsSync(LERNA_JSON)
}

module.exports = async function publishToNPM(options) {
  ...
  const nextVersion = getNextVersion(options, {lastVersion, releaseType})
  console.log('Next version will be', nextVersion.parsed.version)

  if (options.dryRun) {
    console.log('Dry run, exiting without changelog, NPM, or git changes')
  } else {
    await writeChangelog(options, {repository, commits, lastVersion, nextVersion})
    publishToNPM(options, {nextVersion})
    tagAndPushGitRepo(options, {nextVersion, repository})
  }
}
