...
function checkGitHubCredentials() {
  if (!process.env.GH_TOKEN) bail('GitHub not authenticated')
}
...
async function publishReleaseToGitHub(options, data) {
  ...
  await githubLib.createRelease(process.env.GH_TOKEN, repository, {
...
module.exports = async function publishToNPM(options) {
  ...
  if (options.dryRun) {
    console.log('Dry run, exiting without NPM or GitHub changes')
  } else {
    await publishToNPM(options, {nextVersion})
