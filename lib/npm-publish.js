...
const PRERELEASE_SUFFIX = process.env.HULK_PRERELEASE_SUFFIX || 'alpha'
...
function checkPublishConditions(options) {
  const {requiredBranch, requiredNodeVersion} = options
  const {CI, GITHUB_REF} = process.env

  if (!CI) return

  if (GITHUB_REF !== `refs/heads/${requiredBranch}`)
    bail(`Can only publish from ${requiredBranch} but was "${GITHUB_REF}"`)
  if (!process.version.startsWith(requiredNodeVersion))
    bail(`Can only publish from ${requiredNodeVersion} but was ${process.version}`)
}
...
function getNextVersion(options, {lastVersion, releaseType, forceMajor}) {
  ...
  // treat 0.x bumps as one level down bumps
  if (is0DotX && releaseIncrement === 'preminor') releaseIncrement = 'prepatch'
  if (is0DotX && releaseIncrement === 'minor') releaseIncrement = 'patch'
  ...
  const raw = semver.inc(lastVersion.parsed, releaseIncrement, PRERELEASE_SUFFIX)
  const parsed = semver.parse(raw)
  return {hash, parsed, releaseIncrement}
}
...
function checkCommitHasBeenPushed() {
  const primaryBranch = exec('git rev-parse --verify origin/main').code === 0 ? 'main' : 'master'
  const head = exec('git rev-parse HEAD').stdout.trim()
  const hashInRevList = exec(`git rev-list origin/${primaryBranch} | grep ${head}`)
...
function publishPackageToNPM(options, {nextVersion}) {
  ...
  if (options.lerna) {
    ...
    try {
      execSync(`lerna publish ${lernaArgs.join(' ')}`, { env: process.env, stdio: 'inherit' })
      publishCode = 0
    } catch {}
  } else {
    ...
    try {
      execSync(`npm publish --tag ${npmTag}`, { env: process.env, stdio: 'inherit' })
      publishCode = 0
    } catch {}
...
module.exports = async function npmPublish(options) {
  ...
  console.log('ℹ️   Adjusted release type is', chalk.bold(nextVersion.releaseIncrement), '\n')
