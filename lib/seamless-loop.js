...
async function computeHashes(inputFile, options) {
  ...
  for (const file of fs.readdirSync(CACHE_DIR)) {
    ...
    imageHashes[file] = {
      ...
      hash: img.hash(2),
      n,
      timestamp: (n - 1) / options.numberOfAnalysisFrames,
    }
...
function computeTopHashPairsPass1(imageHashes, {crossfadeDuration, analysisDuration}) {
  const pairs = []
  for (const fileA in imageHashes) {
    for (const fileB in imageHashes) {
...
function computeTopHashPairsPass2(imageHashes, pairs, {numberOfAnalysisFrames}) {
  for (const pair of pairs) {
    ...
    for (let i = numberOfAnalysisFrames; i >= 0; i--) {
      ...
      const discountFactor = Math.pow(0.5, Math.abs(i - numberOfAnalysisFrames / 2))
...
function exportFinalVideo({outputFile, duration, crossfadeDuration}) {
  ...
  shelljs.exec(
    `ffmpeg -i ${TMP_VIDEO_1_PATH} -i ${TMP_VIDEO_2_PATH} -filter_complex "${filters}" -vcodec libx264 -map [outv] ${outputFile}`,
    {silent: true},
  )
}
...
module.exports = async function seamlessLoop(yargsOptions) {
  ...
  console.log(bestPairs.slice(0, 5))
